package streams;

import java.util.Arrays;
import java.util.List;
import java.util.function.BinaryOperator;

public class Reduce1 {

	 public static void main(String[] args) {
		 
		List<Integer> nums = Arrays.asList(1,2,3,4,5,6,7,8,9);
		
		BinaryOperator<Integer> soma = (acc, n) -> acc + n;
		
		int total = nums.stream().reduce(soma).get();
		System.out.println(total);
		
		Integer totalComInteger = nums.stream().reduce(soma).get();
		System.out.println(totalComInteger); //PODE USAR INTEGER SEM PROBLEMAS
		
		Integer total2 = nums.parallelStream().reduce(100, soma);
		System.out.println(total2);
		
		Integer total3 = nums.stream().reduce(100, soma);
		System.out.println(total3);
		
		
		//RESULTADO FOI UM Optional<Interger>...
		nums.stream()
			.filter(n -> n > 5)
			.reduce(soma)
			.ifPresent(System.out::print);
								
	}
	
}
